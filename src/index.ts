...
async function calculateAlphaMap(imageBuffer: Buffer): Promise<Float32Array> {
...
export class WatermarkRemover {
  ...
  private bg48Buffer: Buffer;
  private bg96Buffer: Buffer;
  ...
  async removeWatermark(inputPath: string, outputPath: string, options?: { jpegQuality?: number; webpQuality?: number; pngCompressionLevel?: number }): Promise<string> {
...
Output formats:
  ...
  async (params: RemoveWatermarkInput) => {
    try {
